class SMViewer{
	...
	update(items, elapsed){
		...
		if(items.length > 0){
			switch(this.mode){
				case 0:
					if(items[this.playing[0]])
						this.tick_am(elapsed, items);
	...
	update_am(items){
		...
		let params = [];
		let h = this.total_height - margin_bottom;
		let w = this.total_width/this.last_num_selected;
	...
	update_sm(items){
		let margin_left = items[0].elements.time_inputs.panel.clientWidth;
		let updated = [];
		let num_selected = 0;
		let select_ind = 0;
		for(let item_ind = 0; item_ind < items.length; item_ind++){
			if(items[item_ind].selected){
				num_selected++;
				let highl = (this.zooming.length == 0 && items[item_ind].inds.includes(this.highlighted));
		...
		let params = [];
		let h = this.total_height/num_selected;
		for(let i = 0; i < updated.length; i++){
			let w = (this.total_width - margin_left)/items[updated[i][0]].num_steps;
			let vp = new ViewPort(margin_left + w*updated[i][1], this.total_height - h*updated[i][2] - h, w + 1, h + 1);
	...
	refresh_am(items){
		this.playing = [];
		this.am_timesteps = [];
		for(let i = 0; i < this.last_num_selected; i++)
			this.am_timesteps.push(0);
	...
	refresh_sm(items){
		let margin_left = items[0].elements.time_inputs.panel.clientWidth;
		let h = this.total_height/this.last_num_selected;
		let offset = .4*h;
		let params = [];
		let select_ind = 0;
		for(let i = 0; i < items.length; i++){
			if(items[i].selected){
				...
				let w = (this.total_width - margin_left)/items[i].num_steps;
	...
	start_rotate_sm(x, y, items){
		let margin_left = items[0].elements.time_inputs.panel.clientWidth;
		let selected_ind = Math.floor(y/(this.total_height/this.last_num_selected));
		let num_selected = 0;
		let ind = -1;
		for(let i = 0; i < items.length; i++){
			if(items[i].selected){
		...
		if(ind >= 0){
			let w = (this.total_width - margin_left)/items[ind].num_steps;
			let step = Math.floor((x - margin_left)/w);
	...
	start_rotate_am(x, y, items){
		...
		this.mouse.x = x;
		this.mouse.y = y;
		let ind = Math.floor(x/(this.total_width/this.last_num_selected));
		let selected = [];
		for(let i = 0; i < items.length; i++){
			if(items[i].selected){
				selected.push(i);
	...
	rotate(x, y, items){
		if(this.rotating.length > 0){
			let dx = .5 * (x - this.mouse.x);
			let dy = .5 * (y - this.mouse.y);
	...
	zoom_am(e, items){
		let ind = Math.floor(e.clientX/(this.total_width/this.last_num_selected));
		let item_ind = this.find_real_ind(ind, items);

		let offset_ind = Math.floor(items[item_ind].offsets.length/2);
		let camvec = [items[item_ind].camera.x - items[item_ind].offsets[offset_ind].x*.025, items[item_ind].camera.y - items[item_ind].offsets[offset_ind].y*.025, items[item_ind].camera.z - items[item_ind].offsets[offset_ind].z*.025];
		let resized = resize(camvec, e.deltaY*.1);
		if(Math.sign(resized[0]) == Math.sign(camvec[0]) && Math.sign(resized[1]) == Math.sign(camvec[1]) && Math.sign(resized[2]) == Math.sign(camvec[2])){
			items[item_ind].camera.x = items[item_ind].offsets[offset_ind].x*.025 + resized[0];
			items[item_ind].camera.y = items[item_ind].offsets[offset_ind].y*.025 + resized[1];
			items[item_ind].camera.z = items[item_ind].offsets[offset_ind].z*.025 + resized[2];
		}
	...
	zoom_sm(e, items){
		if(this.zooming.length <= 0){
			let margin_left = items[0].elements.time_inputs.panel.clientWidth;
			let ind = this.find_real_ind(Math.floor(e.clientY/(this.total_height/this.last_num_selected)), items);
			let step = Math.floor((e.clientX - margin_left)/((this.total_width - margin_left)/items[ind].num_steps));
			this.zooming = [ind, step, items[ind]];
		}

		let offset_ind = Math.floor(items[this.zooming[0]].offsets.length/2);
		let camvec = [items[this.zooming[0]].camera.x - items[this.zooming[0]].offsets[offset_ind].x*.025, items[this.zooming[0]].camera.y - items[this.zooming[0]].offsets[offset_ind].y*.025, items[this.zooming[0]].camera.z - items[this.zooming[0]].offsets[offset_ind].z*.025];
		let resized = resize(camvec, e.deltaY*.1);
		if(Math.sign(resized[0]) == Math.sign(camvec[0]) && Math.sign(resized[1]) == Math.sign(camvec[1]) && Math.sign(resized[2]) == Math.sign(camvec[2])){
			items[this.zooming[0]].camera.x = items[this.zooming[0]].offsets[offset_ind].x*.025 + resized[0];
			items[this.zooming[0]].camera.y = items[this.zooming[0]].offsets[offset_ind].y*.025 + resized[1];
			items[this.zooming[0]].camera.z = items[this.zooming[0]].offsets[offset_ind].z*.025 + resized[2];
		}
	...
	hover(x, y, items){
		if(this.rotating.length > 0)
			this.rotate(x, y, items);
		else if(items.length > 0){
			if(this.mode == 0){
				let select_ind = Math.floor(x/(this.total_width/this.last_num_selected));
				if(select_ind != this.playing[1]){
					let ind = this.find_real_ind(select_ind, items);
				...
				let margin_left = items[0].elements.time_inputs.panel.clientWidth;
				let select_ind = Math.floor(y/(this.total_height/this.last_num_selected));
				let ind = this.find_real_ind(select_ind, items);
				let step_ind = Math.floor((x - margin_left)/((this.total_width - margin_left)/items[ind].num_steps));
	...
	tick_am(elapsed, items){
		if(items.length > 0 && this.playing.length > 0 && !items[this.playing[0]].paused){
			this.am_time_since_step += elapsed*.75;
			if(this.playing.length > 0 && this.rotating.length <= 0 && this.zooming.length <= 0 && items.length > 0 && this.am_time_since_step > this.am_tick_time){
				...
				if(this.am_timesteps[this.playing[1]] >= items[this.playing[0]].end_t || this.am_timesteps[this.playing[1]] < items[this.playing[0]].start_t)
					this.am_timesteps[this.playing[1]] = items[this.playing[0]].start_t;
			}
		}
	}
...
function make_sm_viewer(w, h){
	let smv = new SMViewer(w, h);

	smv.scroll_end_callback = function(){
		if(smv.mode == 1)
			smv.zooming[2].refresh_sm();
		smv.zooming = [];
