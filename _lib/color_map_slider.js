class ColorMapSlider{
	constructor(css_def){
		...
		let body_rect = this.elements.bar.getBoundingClientRect();
	...
	mousemove(e){
		if(this.dragging.left){
			let body_rect = this.elements.bar.getBoundingClientRect();
			let right_rect = this.elements.bars.high.getBoundingClientRect();

			let low_width = min(max(e.clientX - body_rect.left, 0), right_rect.left - body_rect.left);
			...
			let low_label_rect = this.elements.labels.low.getBoundingClientRect();
			let high_label_rect = this.elements.labels.high.getBoundingClientRect();
			let label_left = min(max(low_width - low_label_rect.width/2, 0), low_label_rect.right - body_rect.left + (high_label_rect.left - low_label_rect.right)/2 - low_label_rect.width - 2.5);
			this.elements.labels.low.style.marginLeft = label_left.toString() + 'px';
			let label_right = min(max(right_rect.width - high_label_rect.width/2, 0), body_rect.right - high_label_rect.left + (high_label_rect.left - low_label_rect.right)/2 - high_label_rect.width - 2.5);
			this.elements.labels.high.style.marginRight = label_right.toString() + 'px';
		}
		else if(this.dragging.right){
			let body_rect = this.elements.bar.getBoundingClientRect();
			let left_rect = this.elements.bars.low.getBoundingClientRect();

			let high_width = min(max(body_rect.right - e.clientX, 0), body_rect.right - left_rect.right);
			...
			let low_label_rect = this.elements.labels.low.getBoundingClientRect();
			let high_label_rect = this.elements.labels.high.getBoundingClientRect();
			let label_right = min(max(high_width - high_label_rect.width/2, 0), body_rect.right - high_label_rect.left + (high_label_rect.left - low_label_rect.right)/2 - high_label_rect.width - 2.5);
			this.elements.labels.high.style.marginRight = label_right.toString() + 'px';
			let label_left = min(max(left_rect.width - low_label_rect.width/2, 0), low_label_rect.right - body_rect.left + (high_label_rect.left - low_label_rect.right)/2 - low_label_rect.width - 2.5);
			this.elements.labels.low.style.marginLeft = label_left.toString() + 'px';
		}
	}
...
function make_color_mapper(css_def){
	let mui = new ColorMapSlider(css_def);
