class Axis{
	constructor(num_ticks, detail, p_fpv, c_fpv, v_fpv){
		...
		let angle_step = this.atom_width/detail;
		...
		let sizes = [[1 + large[0], 1 + large[1]], [1 + small[0], 1 + small[1]]];

		for(let i = 0; i < 2; i++, pos_ind += 2*this.p_fpv){
			let cos = Math.cos(i*this.atom_width/2);
			let sin = Math.sin(i*this.atom_width/2);
	...
	draw(u_ModelMatrix, x, y, z, rx, rz, viewport, scale){
		...
		let step = this.atom_width*180/Math.PI;
