class SelectInterface {
    ...
    activate (viewports, mode) {
        ...
        if (this.mode == 0) { this.apply_button.innerHTML = this.button_text[this.click_ind] } else { this.apply_button.innerHTML = this.button_text[this.button_text.length - 1] }
    }
    ...
    hover (x, y) {
        switch (this.state) {
            case 0:
                ...
                break
            case 1:
                if (this.mouse_down) { this.select_points.push([x, y]) }
...
function make_select_interface (w, h, lc, fc) {
    const si = new SelectInterface(w, h, lc, fc)

    si.canvas.onmousedown = function (e) {
        si.click(e.clientX, e.clientY)
        if (si.state != 0) { si.mouse_down = true }
    }
...
class Brush {
    constructor (points, delta) {
        ...
        while (len < sum) {
            len = n * delta
            if (len > sums[i]) { i++ } else {
                const point = [points[i][0] - (len - sums[i - 1]) * slopes[i][0], points[i][1] - (len - sums[i - 1]) * slopes[i][1]]
